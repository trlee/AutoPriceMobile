        ...
        <fieldset>
            <asp:GridView ID="FriendItems" runat="server" DataKeyNames="ItemID" OnRowDeleting="FriendItems_RowDeleting" AutoGenerateColumns="false" EmptyDataText="There are no items to display. Please check back later!" RowStyle-HorizontalAlign="Center" GridLines="None" Width="1300px" OnRowDataBound="FriendItems_RowDataBound">
                ...
                <Columns>
                    ...
                    <asp:TemplateField>
                        <ItemTemplate>
                            <asp:Calendar ID="StartTime" runat="server" Enabled="false" SelectedDate='<%#Bind("Time") %>' VisibleDate='<%#Eval("Time") %>'></asp:Calendar>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField>
                        <ItemTemplate>
                            <asp:Calendar ID="EndTime" runat="server" Enabled="false" SelectedDate='<%#Bind("TimeEnd") %>' VisibleDate='<%#Eval("TimeEnd") %>'></asp:Calendar>
                        </ItemTemplate>
                    </asp:TemplateField>
